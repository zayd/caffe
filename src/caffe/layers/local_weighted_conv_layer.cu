...
Dtype LocalWeightedConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	for (int n=0; n<num_; n++)
	{
		...
		{
			caffe_gpu_mul(K_*N_, x_data, weight+this->blobs_[0]->offset(m), intermediate.mutable_gpu_data());
...
void LocalWeightedConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
	...
	for (int n=0; n<num_; n++)
	{
		...
		if (propagate_down[0])
		{
			...
			col2im_gpu(x_diff, channels_, height_, width_, kernel_size_, pad_,
		...
		/*for (int m=0; m<num_output_; m++)
		{
			...
			repmat_gpu(top_diff+top[0]->offset(n, m), K_, N_, buf_data);
			caffe_gpu_mul(K_*N_, buf_data, x_data, xt_data);
			caffe_gpu_axpby(K_*N_, Dtype(1.0), xt_data, Dtype(1.0), filter_weight_diff);
		}
		...
		if (propagate_down)
		{
			...
			{
				repmat_gpu(top_diff+top[0]->offset(n, m), K_, N_, buf_data);
				caffe_gpu_mul(K_*N_, buf_data, weight+this->blobs_[0]->offset(m), buf_data);
				caffe_gpu_axpby(K_*N_, Dtype(1.0), buf_data, Dtype(1.0), x_diff);
			}
